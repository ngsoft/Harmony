...
$activity_template = [
    "#!/bin/bash",
    "# This is an autogenerated template for an activity\n",
    'case "$1" in'
];
$activity_dest = __DIR__ . "/activity.keys";
        ...
        if(($data = @parse_ini_string($contents, false, INI_SCANNER_RAW))){
            foreach($data as $replace => $new){
                if($file !== "reserved.conf"){
                    ...
                    //activity
                    $activity_template[] = implode("\n", [
                        "    $new)",
                        "        #exit 1 # if key not used, please uncomment that line",
                        '    ;;'
                    ]);
...
$activity_template[] = implode("\n", [
    "    *)",
    "        # Default behaviour to notify the sent key can pass though",
    "        # If no exit value is provided the key will be blocked by the <main> activity",
    "        exit 1",
    "    ;;",
    "esac",
    "exit 0"
...
file_put_contents($activity_dest, implode("\n", $activity_template));
